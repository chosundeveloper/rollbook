    ...
    <main className="min-h-screen bg-slate-100 pb-12 pt-10">
      <section className="mx-auto max-w-5xl space-y-6 px-4">
        <header>
          ...
          <p className="mt-2 text-sm text-slate-500">등록된 셀과 셀장/부셀장/셀원 구성을 한눈에 확인하세요.</p>
        </header>
            ...
            cells.map((cell) => (
              <article key={cell.id} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="flex flex-col gap-2 border-b border-slate-100 pb-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h2 className="text-2xl font-semibold text-slate-900">{cell.name}</h2>
                    {cell.description && <p className="text-sm text-slate-500">{cell.description}</p>}
                  </div>
                  <span className="text-sm text-slate-500">총 {cell.roster.length}명</span>
                </div>
                    ...
                    <tbody>
                      {cell.roster.map((entry, index) => (
                        <tr key={`${cell.id}-${entry.member?.id ?? index}`} className="border-t border-slate-100">
                          ...
                          <td className="py-2 text-slate-500">{entry.member?.role ?? "-"}</td>
                          <td className="py-2">
